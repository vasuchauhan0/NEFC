...
const numberToEnglish = (n: number): string => {
  ...
  if (hundreds > 0) parts.unshift(translate3Digit(hundreds));
  remaining = Math.floor(remaining / 1000);
  const thousands = remaining % 100;
  if (thousands > 0) parts.unshift(translate3Digit(thousands) + ' Thousand');
  remaining = Math.floor(remaining / 100);
  const lakhs = remaining % 100;
  if (lakhs > 0) parts.unshift(translate3Digit(lakhs) + ' Lakh');
  remaining = Math.floor(remaining / 100);
  const crores = remaining;
  if (crores > 0) parts.unshift(translate3Digit(crores) + ' Crore');
...
const PAGE_LAYOUT_CSS = `@page { margin: 0; }`;
...
export default function PassbookModal({ isOpen, onClose, member, company }: PassbookModalProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text('Member Name:', 20, 88); doc.setFont('Helvetica', 'normal'); doc.text(member.name, 55, 88);
    doc.setFont('Helvetica', 'bold'); doc.text('Member ID:', 110, 88); doc.setFont('Helvetica', 'normal'); doc.text(member.id, 145, 88);
    doc.setFont('Helvetica', 'bold'); doc.text('Email Address:', 20, 98); doc.setFont('Helvetica', 'normal'); doc.text(member.email, 55, 98);
    doc.setFont('Helvetica', 'bold'); doc.text('Phone Number:', 110, 98); doc.setFont('Helvetica', 'normal'); doc.text(member.phone, 145, 98);
    doc.setFont('Helvetica', 'bold'); doc.text('Home Branch:', 20, 108); doc.setFont('Helvetica', 'normal'); doc.text('Mohan Nagar Branch', 55, 108);
    doc.setFont('Helvetica', 'bold'); doc.text('Branch City:', 110, 108); doc.setFont('Helvetica', 'normal'); doc.text(member.city, 145, 108);
    doc.setFont('Helvetica', 'bold'); doc.text('Nominee Name:', 20, 118); doc.setFont('Helvetica', 'normal');
    doc.text(member.nomineeName ? `${member.nomineeName} (${member.nomineeRelation || 'N/A'})` : 'N/A', 55, 118);
    doc.setFont('Helvetica', 'bold'); doc.text('Member Since:', 110, 118); doc.setFont('Helvetica', 'normal'); doc.text(member.memberSince || 'N/A', 145, 118);
    doc.setFont('Helvetica', 'bold'); doc.text('Address / City:', 20, 128); doc.setFont('Helvetica', 'normal'); doc.text(member.city || 'N/A', 55, 128);
    doc.setFont('Helvetica', 'bold'); doc.text('Account Status:', 110, 146);
    doc.setFont('Helvetica', 'bold'); doc.setTextColor(16, 124, 65); doc.text(member.status.toUpperCase(), 145, 146);
    ...
    doc.text('Active Deposits Held:', 22, 182); doc.setFont('Helvetica', 'bold'); doc.text(`${activeInvestments.length} Active Accounts`, 85, 182);
    doc.setFont('Helvetica', 'normal'); doc.text('Total RD Paid Accumulation:', 22, 192); doc.setFont('Helvetica', 'bold'); doc.text(`INR ${totalRDPaid.toLocaleString('en-IN')}.00`, 85, 192);
    doc.setFont('Helvetica', 'normal'); doc.text('Total FD Booking Deposits:', 22, 202); doc.setFont('Helvetica', 'bold'); doc.text(`INR ${totalFDPaid.toLocaleString('en-IN')}.00`, 85, 202);
    doc.setDrawColor(226, 232, 240); doc.line(22, 208, 188, 208);
    doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
    ...
    doc.setDrawColor(203, 213, 225); doc.rect(142, 235, 48, 22, 'S');
    doc.setTextColor(148, 163, 184); doc.setFontSize(7); doc.text('BRANCH SEAL & SIG', 151, 248);
    ...
    activeInvestments.forEach((inv, pageIdx) => {
      ...
      doc.setDrawColor(203, 213, 225); doc.rect(15, 38, 180, 24, 'S');
      doc.setTextColor(TEXT_MAIN[0], TEXT_MAIN[1], TEXT_MAIN[2]);
      doc.setFont('Helvetica', 'bold'); doc.setFontSize(8);
      doc.text('Scheme Catalog ID:', 20, 44); doc.setFont('Helvetica', 'normal'); doc.text(inv.schemeId, 50, 44);
      doc.setFont('Helvetica', 'bold'); doc.text('Scheme Categorization:', 110, 44); doc.setFont('Helvetica', 'normal');
      doc.text(isRD ? 'Recurring Deposit (RD)' : 'Fixed Deposit (FD)', 146, 44);
      doc.setFont('Helvetica', 'bold'); doc.text('Contract Amount:', 20, 50); doc.setFont('Helvetica', 'normal');
      doc.text(isRD ? `INR ${inv.amount.toLocaleString('en-IN')}/mo` : `INR ${inv.amount.toLocaleString('en-IN')}`, 50, 50);
      doc.setFont('Helvetica', 'bold'); doc.text('Interest Rate Yield:', 110, 50); doc.setFont('Helvetica', 'normal');
      doc.text(`${inv.interestPct.toFixed(1)}% p.a.`, 146, 50);
      doc.setFont('Helvetica', 'bold'); doc.text('Booking Start Date:', 20, 56); doc.setFont('Helvetica', 'normal'); doc.text(inv.startDate, 50, 56);
      doc.setFont('Helvetica', 'bold'); doc.text('Term Maturity Date:', 110, 56); doc.setFont('Helvetica', 'normal'); doc.text(inv.maturityDate, 146, 56);
      ...
      const rows = isRD ? getRDTransactions(inv) : [
        { index: 1, date: inv.startDate, particulars: 'F-1', type: 'CR', amount: inv.amount, balance: inv.amount }
      ];

      let yOffset = tableTopY + 8;
      doc.setFont('Helvetica', 'normal'); doc.setFontSize(8);
      rows.forEach((row, rIdx) => {
        if (rIdx % 2 === 1) { doc.setFillColor(248, 250, 252); doc.rect(15, yOffset, 180, 8, 'F'); }
        doc.setDrawColor(226, 232, 240); doc.line(15, yOffset + 8, 195, yOffset + 8);
        doc.setTextColor(TEXT_MAIN[0], TEXT_MAIN[1], TEXT_MAIN[2]);
        doc.text(String(row.index), 21, yOffset + 5.5);
        doc.text(row.date, 38, yOffset + 5.5);
        doc.setFont('Helvetica', 'bold'); doc.text(row.particulars, 75, yOffset + 5.5);
        doc.setFont('Helvetica', 'normal');
      ...
      doc.setDrawColor(226, 232, 240); doc.line(15, ledgerSummaryY, 195, ledgerSummaryY);
      doc.setTextColor(GRAY_DARK[0], GRAY_DARK[1], GRAY_DARK[2]);
      doc.setFont('Helvetica', 'bold'); doc.setFontSize(9);
    ...
    triggerIframePrint('nefc_row_receipt_iframe', `
      ...
      <html>
        <head>
          ...
          <style>
            body { font-family: Arial, sans-serif; margin: 15mm; background-color: white !important; color: black !important; }
            ...
            .frow td { text-align: center; font-weight: bold; font-size: 11px; padding-top: 25px; border-top: 1px dashed #000; margin-top: 30px; }
            @page { margin: 0; }
          </style>
        </head>
        <body>
          <table width="100%" cellpadding="10" cellspacing="10" class="renewal_table">
            <tr class="renewl-head"><td colspan="6" class="renewal">RENEWAL RECEIPT</td></tr>
            <tr class="prow">
        ...
        <div class="print-page-break">
          ...
          <table id="policy_style" class="fd-policy">
            ...
            <tr>
              ...
              <td colspan="3">: ${member.city || 'N/A'}</td>
            </tr>
            <tr>
              <td style="font-weight: 600; background-color: #f7f9fa;">Nominee Name</td>
              <td colspan="3">: ${member.nomineeName ? `${member.nomineeName} (${member.nomineeRelation || 'N/A'})` : 'N/A'}</td>
            </tr>
            <tr>
              ...
              ${paddedChunk.map((t) => {
                ...
                return `
                  <tr>
                    ...
                    <td class="wwlate" style="font-family: monospace; font-weight: bold;">${isPadded ? '--' : formatAmountRaw(t.amount)}</td>
                    <td class="ww" style="font-family: monospace; font-weight: bold;">${isPadded ? '--' : formatAmountRaw(t.balance || t.amount)}</td>
                  </tr>
                `;
              }).join('')}
            </tbody>
    ...
    triggerIframePrint('nefc_print_iframe', `
      ...
      <html>
        <head>
          ...
          <style>
            body { font-family: Arial, sans-serif; margin: 10mm; padding: 0; background-color: white !important; color: black !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            ...
            .rd-ledger-list th { background-color: #000 !important; color: #fff !important; padding: 8px 10px; border: 1px solid #000; text-align: center; font-weight: bold; }
            ...
            .header-left h1 { font-size: 20px; margin: 0; font-weight: bold; text-transform: uppercase; letter-spacing: -0.5px; }
            .header-left span { font-size: 9px; letter-spacing: 0.8px; color: #555; font-weight: bold; }
            ...
            .seal-box { border: 1px solid #000; padding: 12px 20px; font-size: 9px; font-weight: bold; text-align: center; border-radius: 4px; background-color: #fff; }
            @page { margin: 0; }
          </style>
        </head>
        <body>${printPagesHTML}</body>
      </html>
    `);
  };

  // ─── PRINT DETAILS ONLY ───────────────────────────────────────────────────
  // FD → prints formal FD Certificate
  // RD → prints plain details table (unchanged)
  const handlePrintDetailsOnly = () => {
    if (!isRDSelected) {
      // ── FD Certificate ────────────────────────────────────────────────────
      const maturityAmt = getMaturityAmount(activeInv);
      const interestEarned = maturityAmt - activeInv.amount;
          ...
          <head>
            ...
            <style>
              * { box-sizing: border-box; margin: 0; padding: 0; }
              ...
              .cert-meta span { font-weight: bold; }
              .cert-proclamation {
                font-size: 10pt;
                text-align: center;
                color: #333;
              ...
              .maturity-box .mat-label {
                font-size: 10pt;
                font-weight: bold;
                color: #0a5c36;
              }
              .maturity-box .mat-amount {
                font-size: 16pt;
                font-weight: bold;
                color: #0a5c36;
              }
              .maturity-box .mat-words {
                font-size: 8pt;
                color: #555;
                font-style: italic;
                margin-top: 2px;
              ...
              .cert-terms ol { padding-left: 14px; }
              .cert-signatures {
                display: flex;
                justify-content: space-between;
            ...
            <div class="page">
              <div class="cert-border">
                ...
                <div class="cert-header">
                  ...
                  <div class="org-sub">Registered Co-Operative Society · Securities Division</div>
                  <div class="org-addr">${company.address} &nbsp;|&nbsp; ${company.email} &nbsp;|&nbsp; ${company.phone}</div>
                </div>

                <!-- Title Banner -->
                <div class="cert-title-bar">Fixed Deposit Certificate</div>

                <!-- Cert No & Date -->
                <div class="cert-meta">
                  <div>Certificate No: <span>${activeInv.id}</span></div>
                  <div>Issue Date: <span>${activeInv.startDate}</span></div>
                  <div>Branch: <span>Mohan Nagar</span></div>
                </div>
                ...
                <div class="cert-proclamation">
                  ...
                  (Member ID: <strong>${member.id}</strong>) has deposited a sum of
                  <span class="amount-line">&#8377; ${formatAmountRaw(activeInv.amount)}</span>
                  <em style="font-size:8.5pt;">(${numberToEnglish(activeInv.amount)})</em><br/>
                  under the Fixed Deposit Scheme <strong>${activeInv.schemeId}</strong>
                  for a tenure of <strong>${activeInv.durationYears} Year${activeInv.durationYears > 1 ? 's' : ''}</strong>
                  at an interest rate of <strong>${activeInv.interestPct.toFixed(1)}% per annum</strong>.
                </div>
                ...
                <table class="cert-table">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="val" colspan="3">${member.nomineeName ? `${member.nomineeName} (${member.nomineeRelation || 'N/A'})` : 'N/A'}</td>
                    </tr>
                    <tr>
                      <td class="lbl">Scheme / Plan</td>
                    ...
                    <tr>
                      ...
                      <td class="val">${activeInv.durationYears} Year${activeInv.durationYears > 1 ? 's' : ''} (${activeInv.durationYears * 12} Months)</td>
                    ...
                    <tr>
                      ...
                      <td class="val">${activeInv.interestPct.toFixed(1)}% p.a.</td>
                    </tr>
                    <tr>
                      <td class="lbl">Interest Earned</td>
                      <td class="val">&#8377; ${formatAmountRaw(interestEarned)}</td>
                      <td class="lbl">Phone</td>
                      <td class="val">${member.phone}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div class="maturity-box">
                  <div>
                    <div class="mat-label">Total Maturity Amount</div>
                    <div class="mat-words">${numberToEnglish(Math.floor(maturityAmt))}</div>
                  </div>
                  <div class="mat-amount">&#8377; ${formatAmountRaw(maturityAmt)}</div>
                </div>
                ...
                <div class="cert-terms">
                  ...
                  <ol>
                    <li>This certificate is non-transferable and valid only for the member named above.</li>
                    <li>Premature withdrawal is subject to applicable penalty as per society norms.</li>
                    <li>This is a computer-generated certificate. Physical signature required for validation.</li>
                    <li>In case of loss, a duplicate certificate may be issued upon written request at the branch.</li>
                  </ol>
                </div>
                ...
                <div class="cert-signatures">
                  ...
                  <div class="seal-box">BRANCH<br/>OFFICIAL<br/>SEAL</div>
                  <div class="sig-block">
                    <div class="sig-line"></div>
    ...
    // ── RD: plain details table (unchanged) ──────────────────────────────────
    triggerIframePrint('nefc_print_details_iframe', `
      <!DOCTYPE html>
      <html>
        <head>
          ...
          <style>
            body { font-family: Arial, sans-serif; margin: 20mm 15mm; padding: 0; background-color: white !important; color: black !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            .details-table { width: 100%; border-collapse: collapse; font-size: 14px; font-family: Arial, sans-serif; }
            .details-table td { padding: 8px 10px; vertical-align: top; }
            .lbl { font-weight: bold; width: 25%; white-space: nowrap; }
            .val { width: 25%; }
            @page { margin: 0; }
          </style>
        </head>
  ...
  const handlePrintLedgerPageOnly = () => {
    ...
    const paddedChunk = activeChunk.map((t) => {
      ...
      if (isExcluded) return { date: '', particulars: '', month: '', lateFees: '', amount: 0, balance: 0, isEmpty: true };
      return t;
    });
    triggerIframePrint('nefc_print_ledger_page_iframe', `
      ...
      <html>
        <head>
          ...
          <style>
            body { font-family: 'Courier New', Courier, monospace; font-size: 13px; font-weight: bold; margin: 20mm 15mm; padding: 0; background-color: white !important; color: black !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            .ledger-table { width: 100%; border-collapse: collapse; border: none; }
            .ledger-table td { padding: 8px 0; border: none; }
  ...
  while (activePaddedChunk.length < 6) {
    activePaddedChunk.push({ index: activePaddedChunk.length + 1, date: '--', particulars: '--', month: '--', lateFees: '--', amount: 0, balance: 0, type: '--', isEmpty: true } as any);
  }
      ...
      <div className="bg-slate-800 w-full max-w-5xl rounded-3xl overflow-hidden flex flex-col shadow-2xl h-[95vh] border border-slate-700">
        ...
        <div className="bg-slate-900 border-b border-slate-700 px-4 py-3 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              title={isRDSelected ? "Print only customer and policy details cover page" : "Print FD Certificate"}
            >
              <BookOpen size={14} className="text-amber-400" />
              <span>{isRDSelected ? 'Print Details Only' : 'Print FD Certificate'}</span>
            </button>
            <button
              ...
              <div className="overflow-x-auto">
                <table id="policy_style" className="w-full border-collapse border border-slate-950 text-xs">
                  <tbody>
                    ...
                    <tr className="border-b border-slate-950">
                      <td className="p-2 sm:p-2.5 font-bold bg-slate-100 border-r border-slate-950">Address / City</td>
                      <td className="p-2 sm:p-2.5" colSpan={3}>: {member.city || 'N/A'}</td>
                    </tr>
                    <tr className="border-b border-slate-950">
                      <td className="p-2 sm:p-2.5 font-bold bg-slate-100 border-r border-slate-950">Nominee Name</td>
                      <td className="p-2 sm:p-2.5" colSpan={3}>: {member.nomineeName ? `${member.nomineeName} (${member.nomineeRelation || 'N/A'})` : 'N/A'}</td>
                    </tr>
                    <tr className="border-b border-slate-950">
        ...
        <div className="bg-slate-900 border-t border-slate-700 px-4 py-3 flex items-center justify-between text-xs text-slate-400">
          <div className="flex items-center gap-1">
            <button
              onClick={() => { setCurrentPage(prev => Math.max(1, prev - 1)); setExpandedReceiptIdx(null); }}
            ...
            <button
              onClick={() => { setCurrentPage(prev => Math.min(totalSimPages, prev + 1)); setExpandedReceiptIdx(null); }}
